...
Element.prototype.__defineGetter__('border', function() {
  return this._border;
});

Element.prototype.__defineSetter__('border', function(border) {
  if (border != null) {
...
Element.prototype.render = function() {
  ...
  var lines = this.screen.lines
    ...
    , i
    , bch = this.ch
    , blend = typeof(this.style.opacity) === 'number' && this.style.opacity < 1
    , alpha = 1 - this.style.opacity;
    ...
    if (blend) {
     for (y = Math.max(yi, 0); y < yl; y++) {
       ...
       for (x = Math.max(xi, 0); x < xl; x++) {
         if (!lines[y][x]) break;
         lines[y][x][0] = colors.blend(attr, lines[y][x][0], alpha);
         // lines[y][x][1] = bch;
         lines[y].dirty = true;
       }
     }
    ...
    for (x = xi; x < xl; x++) {
      ...
      if (ch === '\n') {
        ...
        for (; x < xl; x++) {
          ...
          if (blend) {
            lines[y][x][0] = colors.blend(attr, lines[y][x][0], alpha);
            if (content[ci - 1]) lines[y][x][1] = ch;
            lines[y].dirty = true;
          } else {
